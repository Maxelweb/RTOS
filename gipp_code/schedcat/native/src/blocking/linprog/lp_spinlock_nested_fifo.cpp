...
class NestedFifoILP : protected LinearProgram
{
	...
	const bool rnlp; // assume RNLP contention control
	const bool nesting_aware; // assume RNLP is nesting-aware

	// for each resource, the set of resources that may be nested
	hashmap<unsigned int, hashset<unsigned int> > trans_nested;
	...
	bool may_be_nested_in(unsigned int res_q, unsigned int res_p)
	{
		return trans_nested[res_p].find(res_q) != trans_nested[res_p].end();
	}
...
NestedFifoILP::NestedFifoILP(
  ...
  : i(task_under_analysis),
	...
	max_resource(0),
	rnlp(with_rnlp),
	nesting_aware(rnlp_nesting_aware),
	trans_nested(tsk_cs.get_transitive_nesting_relationship())
{
	std::cout << "**************** ANALYSIS FOR " << ti << " **************** " << std::endl;
	std::cout << "RNLP: " << rnlp;
	if (nesting_aware)
		std::cout << " (nesting aware)";
	std::cout <<std::endl;
...
void NestedFifoILP::set_objective()
{
	...
	enumerate(taskset, tsk, x)
	{
		    ...
		    tsk->get_cluster() != ti.get_cluster())
		{
			...
			std::cout << std::endl << std::endl << *tsk << std::endl;

			unsigned int cs_index;
			const CriticalSections& tx_cs = taskset_cs[x].get_cs();

			std::cout << "num_jobs=" << tsk->get_max_num_jobs(ti.get_response())
				<< " num_cs=" << tx_cs.size() << std::endl;
	...
	unsigned int k, unsigned int q, const LockSet& serializing)
{
	...
	std::cout << "C6 for k=" << k << " q=" << q << " sr=" << serializing << std::endl;
			...
			if (cs->resource_id == q)
			{
				...
				if (is_subset_of(serializing, outer_locks[x][cs_index]))
				{
					std::cout << "   -> " << serializing
						<< " is subset of " << outer_locks[x][cs_index]
						<< std::endl;
					/* for all instances of *cs while a job of Ti is pending */
					enumerate_cs_instances(*tx, tx_cs, cs_index, v)
					{
	...
	foreach_local_task(taskset, ti, tx)
	{
		...
		enumerate(tx_cs, cs, cs_index)
		{
			...
			if (cs->resource_id == q)
			{
				...
				{
					std::cout << "       :: " << serializing
						<< " is disjoint with " << outer_locks[x][cs_index]
						<< std::endl;
	...
	foreach_task_not_in_cluster(taskset, k, tx)
	{
		...
		enumerate(tx_cs, cs, cs_index)
		{
			...
			if (cs->resource_id == q)
			{
				...
				{
					std::cout << "       :: " << serializing
						<< " is np-disjoint with " << outer_locks[x][cs_index]
						<< std::endl;
					std::cout << "       :: " << serializing
						<< " is av-disjoint with " << guaranteed_held_cs_path[x][cs_index]
						<< std::endl;
...
void NestedFifoILP::precompute_guaranteed_held()
{
	...
	for (unsigned int q = 0; q <= max_resource; q++)
	{
		std::cout << "guaranteed[" << q << "] = "
			<< guaranteed_held_on_path[q]
			<< std::endl;
	}
}
...
void NestedFifoILP::add_arrival_blocking_constraints()
{
	...
	std::cout << std::endl << std::endl << "Arrival blocking for: " << ti << std::endl;
	...
	bool rnlp_nesting_aware)
{
	...
	{
		NestedFifoILP ilp(info, tsk_cs, i, rnlp, rnlp_nesting_aware);
		(*results)[i] = ilp.solve();
	}
