...
class Test_cpp_non_nested_analysis(unittest.TestCase):
    ...
    @unittest.skipIf(not schedcat.locking.bounds.lp_cpp_available, "no native LP solver available")
    def test_rnlp(self):
        res = lb.apply_pfp_nested_fifo_spinlock_bounds(self.ts, rnlp=True)
    ...
    @unittest.skipIf(not schedcat.locking.bounds.lp_cpp_available, "no native LP solver available")
    def test_rnlp_na(self):
        res = lb.apply_pfp_nested_fifo_spinlock_bounds(self.ts, rnlp=True,
            rnlp_nesting_aware=True)
...
#        self.t3.critical_sections.add_outermost(4, 3)
    ...
    def test_apply_nested_fifo_bounds(self):
        ...
        return
        nyi_default = 2**32-1
        self.assertEqual(self.t1.blocked, nyi_default)
        self.assertEqual(self.t2.blocked, nyi_default)
        self.assertEqual(self.t3.blocked, nyi_default)
